...
async function saveGameProgress(type,data){must();if(!uid())throw new Error('Sign in first');return setDoc(doc(db,'patients',uid(),'gameProgress',type),{...data,type,patientId:uid(),updatedAt:serverTimestamp()},{merge:true})}
async function loadGameProgress(patientId){must();const snap=await getDocs(collection(db,'patients',patientId,'gameProgress')),result={};snap.forEach(d=>result[d.id]={...d.data(),type:d.id});return result}
...
async function saveStory(data,patientId=uid()){must();if(!patientId)throw new Error('Select a patient first.');let id=String(data.id||Date.now());await setDoc(doc(db,'patients',patientId,'stories',id),{...data,id,patientId,updatedAt:serverTimestamp()},{merge:true});return id}
async function loadStories(patientId){must();const snap=await getDocs(collection(db,'patients',patientId,'stories'));return snap.docs.map(item=>({id:item.id,...item.data()}))}
async function deleteStory(patientId,storyId){must();return deleteDoc(doc(db,'patients',patientId,'stories',String(storyId)))}
...
async function deletePatientProfile(profile){await requireRole('caretaker');if(!profile?.id)throw new Error('Patient profile not found.');const snap=await getDoc(doc(db,'patientProfiles',profile.id));if(!snap.exists()||snap.data().caretakerId!==uid())throw new Error('You can delete only your own patient dashboard.');if(profile.inviteCode)await deleteDoc(doc(db,'patientInvites',profile.inviteCode)).catch(()=>{});if(profile.patientId){const links=await getDocs(query(collection(db,'careLinks'),where('patientId','==',profile.patientId)));await Promise.all(links.docs.map(link=>deleteDoc(link.ref).catch(()=>{})))}await deleteDoc(doc(db,'patientProfiles',profile.id))}
async function saveReminder(patientId,data,id=String(Date.now())){must();await setDoc(doc(db,'patients',patientId,'reminders',String(id)),{...data,id:String(id),patientId,updatedAt:serverTimestamp()},{merge:true});return String(id)}
async function loadReminders(patientId){must();const snap=await getDocs(collection(db,'patients',patientId,'reminders'));return snap.docs.map(d=>({id:d.id,...d.data()})).sort((a,b)=>String(a.time||'').localeCompare(String(b.time||'')))}
async function deleteReminder(patientId,id){must();return deleteDoc(doc(db,'patients',patientId,'reminders',String(id)))}
async function saveMedicalCheck(patientId,data,id=String(Date.now())){await requireRole('doctor');if(!patientId)throw new Error('Select an assigned patient first.');await setDoc(doc(db,'patients',patientId,'appointments',String(id)),{...data,id:String(id),patientId,doctorId:uid(),updatedAt:serverTimestamp()},{merge:true});return String(id)}
async function loadMedicalChecks(patientId){must();const snap=await getDocs(collection(db,'patients',patientId,'appointments'));return snap.docs.map(d=>({id:d.id,...d.data()})).sort((a,b)=>`${a.date||''} ${a.time||''}`.localeCompare(`${b.date||''} ${b.time||''}`))}
...
window.CareFirebase={configured,ready,get user(){return auth?.currentUser},get profile(){return currentProfile},signUp,signIn,signOut:()=>{must();return signOut(auth)},resetPassword:e=>{must();return sendPasswordResetEmail(auth,e)},createPatientProfile,caretakerPatients,deletePatientProfile,saveReminder,loadReminders,deleteReminder,saveMedicalCheck,loadMedicalChecks,createRelativeInvite,savePatientSettings,loadOwnPatientProfile,loadPatientProfile,linkedPatientProfiles,saveGame:d=>{must();return setDoc(doc(db,'patients',uid(),'gameReports','summary'),{patientId:uid(),sessions:arrayUnion(d),sessionCount:increment(1),lastSession:d,updatedAt:serverTimestamp()},{merge:true})},saveAssessment:d=>savePatientRecord('assessments',d),saveNotification:d=>savePatientRecord('notifications',d),saveEmergency:(d,patientId=uid())=>{must();return setDoc(doc(db,'patients',patientId,'private','emergency'),{...d,updatedAt:serverTimestamp()},{merge:true})},saveStory,loadStories,deleteStory,saveChat:d=>savePatientRecord('chatHistory',d),linkedPatientIds,loadReports,saveGameProgress,loadGameProgress,loadAssessments,loadChat,loadPatientNotifications,replyToPatient,loadEmergency,deleteChat,patientLinks,approveLink};
